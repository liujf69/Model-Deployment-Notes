...
def prepare_model(
    ...
    if per_channel_quantization:
        quant_desc_input = QuantDescriptor(calib_method=calibrator) # QuantDescriptor定义Tensor应该如何量化
        quant_nn.QuantConv2d.set_default_quant_desc_input(quant_desc_input) # quant_desc_input 表示激活值量化
        quant_nn.QuantLinear.set_default_quant_desc_input(quant_desc_input) 
    else:
        ## Force per tensor quantization for onnx runtime
        quant_desc_input = QuantDescriptor(calib_method=calibrator, axis=None) # axis = None表示使用per tensor scale
        quant_nn.QuantConv2d.set_default_quant_desc_input(quant_desc_input)
        quant_nn.QuantConvTranspose2d.set_default_quant_desc_input(quant_desc_input)
        quant_nn.QuantLinear.set_default_quant_desc_input(quant_desc_input)

        quant_desc_weight = QuantDescriptor(calib_method=calibrator, axis=None)
        quant_nn.QuantConv2d.set_default_quant_desc_weight(quant_desc_weight) # quant_desc_weight 表示使用权重值量化
